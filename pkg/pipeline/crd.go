...
var reParametersType = regexp.MustCompile(`Parameters\d*$`)
var reObservationType = regexp.MustCompile(`Observation\d*$`)
...
func (cg *CRDGenerator) Generate(cfg *config.Resource) error {
	...
	for _, t := range typeList {
		if reParametersType.MatchString(strings.TrimPrefix(t.Obj().Name(), cfg.Kind)) {
			parametersType = t.Obj().Name()
		}
		if reObservationType.MatchString(strings.TrimPrefix(t.Obj().Name(), cfg.Kind)) {
			observationType = t.Obj().Name()
		}
	}
	if parametersType == "" {
		return errors.New("cannot find top level parameters type")
	}
	if observationType == "" {
		return errors.New("cannot find top level observation type")
	}
