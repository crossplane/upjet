...
type Value struct {
	...
	name string
	// omit is the omission policy for the tag if one has been set.
	omit *Omit
...
func build(key Key, opts ...Option) *Value {
	...
	if v.inline && ptr.Deref(v.omit, NotOmitted) != NotOmitted {
...
func parse(key Key, value string) (*Value, error) {
	...
	// inline cannot be combined with omitempty.
	if v.inline && v.omit != nil {
...
func (v *Value) AlwaysOmitted() bool {
	return ptr.Deref(v.omit, NotOmitted) == OmitAlways
}
